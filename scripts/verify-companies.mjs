...
ok(await p.locator('.ph-co').count() === 1, 'the profile lists one company')
await p.locator('.ph-addbtn').click(); await p.waitForTimeout(1000)
...
await p.locator('.tb-menu-item', { hasText: 'My companies' }).click()
await p.waitForTimeout(1200)
ok(await p.locator('.ph-co').count() === 2, 'two companies now')
await p.locator('.ph-co').first().locator('.btn.primary').click()
await p.waitForTimeout(1200)

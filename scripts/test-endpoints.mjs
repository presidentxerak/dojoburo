...
const ENDPOINTS = [
  ...
  ['community', 'api/community.ts', [
    ...
    ['GET', '/api/community?action=members'],
    ['GET', '/api/community?action=member&id=pas-un-id'],
    ['GET', '/api/community?action=leaderboard'],
    ['GET', '/api/community?action=events'],
    ['GET', '/api/community?action=unread'],
...
console.log('\n--- sans base, personne ne prétend avoir réussi ---------------')
{
  ...
  const WRITES = [
    ...
    ['community', 'api/community.ts', 'POST', '/api/community?action=profile', { name: 'Nora', bio: 'Je découvre.' }],

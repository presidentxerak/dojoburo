...
export default async function handler(req: IncomingMessage, res: ServerResponse): Promise<void> {
  ...
  for (const row of pending.rows) {
    try {
      const r = await settleOnLedger(row.destination, Number(row.xrp_amount), row.session_id)
      const ok = r.result === 'tesSUCCESS' && r.validated
      await pool.query(`update settlements set tx_hash = $2, tx_result = $3, status = $4 where session_id = $1`, [
        row.session_id,
        r.hash,
        r.result,
        ok ? 'validated' : 'failed',

...
- THE APP OPENS ON A BOTTOM BAR WITH FOUR BUTTONS, in this order. DOJOBURO is the game. TRAINING is the courses (it used to be called Dojos): the free AI weekend, the full training and the trade trainings, each made of dojo cities with one short lesson per dojo. CLAN is where learners will show what they built (there is no feed yet, and the page says so). PROFILE is progress, badges, the valley map, and the "Your account" card: signing in (email or Google, no password) saves progress online and syncs it across devices; without signing in, everything stays in that browser. The whole app has one dark violet theme. You, Dojobot, are the violet chat bubble at the bottom right.

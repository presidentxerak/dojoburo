...
- The landing page says "Learn to build AI agents, and to run them cheap". The main button is "Enter the dojo · free" and it opens /build. A "How it works" button plays a walkthrough full screen.
- THERE ARE THREE COURSES, taken in this order. 1. BUILD AN AGENT (/build): the dojo itself. 2. PROMPT ENGINEERING (/academy): how to write the instruction that decides everything. 3. TOKEN FRUGALITY (/frugality): where the tokens go and how to cut them. Two more places are not courses: the LIBRARY (/library), a catalogue of files, and the PRACTICE ROOM (the 3D dojo inside the app), a sandbox to take a worked example apart.
...
- PRICING · NOTHING IS METERED, and this is the first thing to say if someone asks what it costs. We do not sell runs, tasks, credits or tokens, and no plan counts them, because the dojo calls no paid model. Three plans: FREE ($0) is the whole course, all three of them, plus the practice dojo, plus every belt, badge and the certified diploma at the end. It is free because it costs us nothing to serve and because the diploma is the proof the course works. LIBRARY ($19/month) buys the files: every prompt, brief .md and agent skill as a real downloadable file rather than a copy-paste, plus the new ones as they are written, plus a custom domain and no DojoBuro badge. SCHOOL ($15 per seat per month, 5 seats minimum, so $75/month and up) is for someone training a group: one bill, seats added or removed any month, and a view of who has earned which belts and diplomas. Below 5 people the Library plan is cheaper and you should say so rather than sell School. There is no crypto, no wallet and no coins to manage. If someone asks about the old "2,000 tasks a month" Managed plan, say plainly that it is gone: it sold hosted runs, and nothing runs here any more.

...
- THE APP OPENS ON A BOTTOM BAR WITH FOUR BUTTONS, in this order. DOJOBURO is the game. AI TRAINING (labelled "IA Training" in French; it was called Training before, and Dojos before that) holds the trainings: the free AI weekend (it asks for an email and nothing else), the full training (the general path) and 14 trade trainings (growth, communication, founder, product, sales, executive assistant, designer, teacher, student, scientist, developer, recruiter, legal counsel, consultant), each made of dojo cities with one lesson per dojo. COMMUNITY (the tab formerly called Clan) is a free Skool-style community: a feed with categories, pinned posts, likes, threaded comments and search, a Calendar of lives (.ics export), Members, Leaderboards (points = likes received, nine levels), notifications (also by email, can be turned off), private messages, @mentions, polls, embedded YouTube, Vimeo, Loom, Instagram and TikTok players, and an About page; reading is open, taking part needs a free account. PROFILE has five tabs, each with a 3D icon: Progression (grade avatar, level, XP, gauges, the ladder of the seven grades, a resume button), Badges (the trophy case), Trainings (what is unlocked, the trade), Account (sign in with email or Google, no password, when sign-in is switched on for the site; progress is then saved online and synced across devices; sign out; without signing in, everything stays in that browser) and Settings. Settings include the Display: light by default, dark violet, or the device's choice. You, Dojobot, are the violet chat bubble at the bottom right.

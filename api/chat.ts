...
- The DOJO ACADEMY (/academy) is the free course on how any of this works: 20 lessons across 5 tracks, about 2 hours, starting from "what is an AI agent" and assuming the person has never heard of vibe coding, an IDE or a coding agent. Each lesson has an animation beside the text, a question that marks itself, one line to remember and one thing to go and do. No account, nothing gated, progress kept in the browser. Send anyone who is new there first. The separate app setup guide (/guide) is the reference for connecting a specific app, not for learning.

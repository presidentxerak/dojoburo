...
// vaut ici aussi). Le français tutoie et emploie le jargon IA anglophone,
...
export const T = {
  ...
  tagline: B('Run your AI studio in a dojo', 'Dirige ton studio IA dans un dojo'),
  pitch: B(
    'Clients walk in with briefs. Give each job the right specialists and just enough tokens. Hit the day\'s revenue before 6 pm.',
    "Des clients entrent avec leurs briefs. Donne à chaque travail les bons spécialistes et juste assez de tokens. Atteins l'objectif du jour avant 18 h.",
  ),
  play: B('Play', 'Jouer'),
  ...
  waiting: B('Waiting clients', 'Clients qui attendent'),
  noneWaiting: B('Nobody is waiting. The next client is on the way.', "Personne n'attend. Le prochain client arrive."),
  ...
  team: B('Your team', 'Ton équipe'),
  pickTeam: B('Pick up to 4 specialists, then give them tokens.', "Choisis jusqu'à 4 spécialistes, puis donne-leur des tokens."),
  level: B('Lv.', 'Niv.'),
  busy: B('Busy', 'Occupé'),
  ...
  overfuel: B('More tokens than needed: little gain, less budget.', 'Plus de tokens que nécessaire : peu de gain, moins de budget.'),
  underfuel: B('Not enough tokens: the work will be rushed.', 'Pas assez de tokens : le travail sera bâclé.'),
  ...
  errEmpty: B('Pick at least one specialist.', 'Choisis au moins un spécialiste.'),
  errMany: B('Four specialists at most.', 'Quatre spécialistes au plus.'),
  ...
  missed: B('Objective missed. Replay the day: you keep what you earned.', "Objectif manqué. Rejoue la journée : tu gardes ce que tu as gagné."),
  ...
  shopLead: B('Spend today\'s cash to make tomorrow easier.', "Dépense la caisse pour rendre demain plus facile."),
  buy: B('Buy', 'Acheter'),
  owned: B('Owned', 'Acquis'),
  train: B('Train', 'Former'),
  maxed: B('Max', 'Max'),
  tutorial1: B('Clients walk in and wait at the front. Tap a client card to read the brief.', 'Les clients entrent et attendent devant. Touche la carte d\'un client pour lire son brief.'),
  tutorial2: B('Pick the specialists the brief needs, then give them tokens. The quality preview updates live.', 'Choisis les spécialistes dont le brief a besoin, puis donne-leur des tokens. La qualité prévue se met à jour en direct.'),
  tutorial3: B('Tokens are your daily budget. Right team, just enough tokens: that is how you hit the objective and keep a frugality bonus.', 'Les tokens sont ton budget du jour. La bonne équipe, juste assez de tokens : c\'est comme ça que tu atteins l\'objectif et gardes un bonus de frugalité.'),
  next: B('Next', 'Suivant'),
  gotIt: B('Let\'s go', "C'est parti"),
...
export const MASTER: Record<'start' | Outcome | 'leave' | 'won' | 'missed', Bi> = {
  start: B('A new day. Choose well, spend little.', 'Un nouveau jour. Choisis bien, dépense peu.'),
  excellent: B('That is how it is done!', "C'est comme ça qu'on fait !"),
  good: B('Good work.', 'Du bon travail.'),
  meh: B('Rushed. More tokens, or a better match?', 'Bâclé. Plus de tokens, ou un meilleur choix ?'),
  failed: B('Wrong specialists for this one.', 'Pas les bons spécialistes pour celui-là.'),
  leave: B('A client waited too long.', 'Un client a trop attendu.'),
  won: B('Objective reached. Well played!', 'Objectif atteint. Bien joué !'),
  missed: B('Tomorrow we try again.', 'On recommence demain.'),
}
...
export const EVENTS: Record<EventKind, { name: Bi; body: Bi }> = {
  ...
  viral: { name: B('You went viral', 'Tu fais le buzz'), body: B('Rewards are 30 % higher today.', 'Les récompenses sont 30 % plus hautes aujourd\'hui.') },
...
export const OFFERS: Record<OfferId, { name: Bi; body: Bi }> = {
  ...
  coffee: { name: B('Coffee machine', 'Machine à café'), body: B('Your team gets tired 40 % more slowly.', "Ton équipe se fatigue 40 % moins vite.") },

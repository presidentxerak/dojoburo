// The three.js half of the landing objects, in its own module ON PURPOSE.
//
// Object3D and Object3DInline already waited for the element to scroll into
// view before mounting a Canvas — but they imported three.js at the top of the
// file, so every visitor downloaded 841 kB of it before the first sentence
// rendered, including on the job-title pages that a search engine reads. The
// wait was for the GPU, not for the network.
//
// Splitting the file moves that fetch behind the same useInView the render was
// already behind. Nothing about the picture changes; it simply stops blocking
// the words.
...
/** The canvas itself · the only thing in the app that pulls three.js in. */
export default function ObjScene({ kind, color, speed = 0.5 }: { kind: string; color: string; speed?: number }) {
  return (

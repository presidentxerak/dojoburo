...
export default function FinanceModule({ dojoId }: ModuleProps) {
  ...
  useEffect(() => {
    let alive = true
    void loadFinance(dojoId).then((p) => { if (alive && p) { setTxns(p.txns); setVatRate(p.vatRate) } })
    return () => { alive = false }
  }, [dojoId])

  const tot = useMemo(() => totals(txns), [txns])
  const months = useMemo(() => byMonth(txns), [txns])
  const cats = useMemo(() => byCategory(txns), [txns])
  const tva = useMemo(() => vat(txns, vatRate), [txns, vatRate])
  const fc = useMemo(() => forecast(txns), [txns])
  ...
  return (
    <div className="ad-body fin-mod">
      <div className="site-toolbar">
        ...
        <div className="site-tb-actions">
          <button className="btn tiny" onClick={exportCsv} disabled={!txns.length}>Exporter CSV</button>
      ...
      {txns.length === 0 ? (
        ...
        <>
          ...
          {/* transactions */}
          <h4 className="brand-h">Transactions ({txns.length})</h4>
          <div className="fin-table">
            {txns.slice(0, 200).map((t) => (
              <div key={t.id} className="fin-trow">
                <span className="fin-date">{t.date.slice(5)}</span>
                <span className="fin-label" title={t.label}>{t.label}</span>
                ...
                <button className="fin-del" onClick={() => del(t.id)} aria-label="Supprimer">✕</button>
              </div>
            ))}
          </div>
          <p className="muted small">Analyse locale : rentabilité par catégorie, TVA estimée et prévision sur la moyenne des 3 derniers mois. Tout reste dans ton navigateur.</p>
        </>
      )}
    </div>
  )

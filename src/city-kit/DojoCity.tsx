...
// ---------------------------------------------------------------------------
// DojoCity · a self-contained, portable 3D isometric city (React + Three.js).
// Zero coupling to any host app: everything it needs comes in through props.
// See ./types for the public API and ./README.md for usage.
import { useMemo, useRef, useState } from 'react'
import { Canvas, useFrame, useThree } from '@react-three/fiber'
import { OrthographicCamera, MapControls, Html, Edges } from '@react-three/drei'
import * as THREE from 'three'
import { buildCity, ZOOM_LEVELS, SPAN, HALF, GRID, CELL, type BuildingSpec, type Lot, type CivicKind } from './cityLayout'
import type { CityCompany, DojoCityProps } from './types'

// Themed colours for the little street walkers (replaces the host's character
// skins so the kit stays dependency-free).
const WALKER_COLORS = ['#7b5cff', '#2f7fd6', '#e0459b', '#1fa563', '#d98c17', '#e0483f', '#00b3a4', '#a855f7', '#f4a261', '#5b6472']
...
function CompanyBuilding({ co, onSelect, labeled }: { co: CityCompany; onSelect: () => void; labeled: boolean }) {
  ...
  const w = 2.6, d = 2.3
  const podiumH = 0.7
  const accent = co.accent || '#2f6bff'
  const sign = useMemo(() => brandSign(co.name, accent), [co, accent])
  return (
    <group
      ...
      <mesh position={[0, podiumH / 2, 0]} castShadow receiveShadow><boxGeometry args={[w + 0.16, podiumH, d + 0.16]} /><meshStandardMaterial color={accent} roughness={0.5} /></mesh>
      ...
      <mesh position={[0, 0.02, 0]} rotation={[-Math.PI / 2, 0, 0]} receiveShadow><planeGeometry args={[w + 1.3, d + 1.3]} /><meshStandardMaterial color={hover ? '#fde68a' : accent} transparent opacity={hover ? 0.9 : 0.16} /></mesh>
      <Html position={[0, podiumH + h + 1.5, 0]} center pointerEvents="none" zIndexRange={[28, 0]}>
        {labeled || hover
          ? <div style={{ ...nameTag, background: accent }}>{co.name}{co.cat ? ` · ${co.cat}` : ''} →</div>
          : <div style={{ ...pinTag, background: accent }}>{co.name[0]}</div>}
      </Html>
    </group>
  )
}
...
// A tiny self-contained low-poly pedestrian (body + head + two swinging legs).
// Bundled so the kit needs no external character system.
function CityWalker({ color }: { color: string }) {
  const legL = useRef<THREE.Mesh>(null)
  const legR = useRef<THREE.Mesh>(null)
  useFrame((state) => {
    const t = state.clock.elapsedTime * 6
    if (legL.current) legL.current.rotation.x = Math.sin(t) * 0.7
    if (legR.current) legR.current.rotation.x = -Math.sin(t) * 0.7
  })
  return (
    <group>
      <mesh position={[0, 0.62, 0]} castShadow><capsuleGeometry args={[0.16, 0.34, 4, 8]} /><meshStandardMaterial color={color} /></mesh>
      <mesh position={[0, 1.02, 0]} castShadow><sphereGeometry args={[0.18, 12, 12]} /><meshStandardMaterial color="#ffe0c2" /></mesh>
      <mesh ref={legL} position={[-0.08, 0.4, 0]}><boxGeometry args={[0.09, 0.34, 0.09]} /><meshStandardMaterial color="#2a2f3a" /></mesh>
      <mesh ref={legR} position={[0.08, 0.4, 0]}><boxGeometry args={[0.09, 0.34, 0.09]} /><meshStandardMaterial color="#2a2f3a" /></mesh>
    </group>
  )
}

// Little pedestrians walking the streets, scaled down to street size.
function Walkers({ show }: { show: boolean }) {
  const items = useMemo(
    () => makeMovers('ped', 14).map((m, i) => ({ m, color: WALKER_COLORS[i % WALKER_COLORS.length], id: `city-npc-${i}` })),
    [],
  )
  const refs = useRef<(THREE.Group | null)[]>([])
  useFrame((state, dt) => {
...
// ---- the scene --------------------------------------------------------------
function CityScene({ level, hqFloors, hqName, hqAccent, companies, onEnter, onGuide, onTip, onSelectCo }: {
  level: number; hqFloors: number; hqName: string; hqAccent: string; companies: CityCompany[]
  onEnter: () => void; onGuide: () => void; onTip: () => void; onSelectCo: (co: CityCompany) => void
}) {
  ...
  const companyByLot = useMemo(() => {
    const map = new Map<string, CityCompany>()
    ...
    let ci = 0
    for (let k = 0; k < candidates.length && ci < companies.length; k += 2) {
      map.set(candidates[k].id, companies[ci++])
    }
    return map
  }, [sorted, reserved, builtRadius, companies])
...
function CompanyFiche({ co, onClose }: { co: CityCompany; onClose: () => void }) {
  const accent = co.accent || '#2f6bff'
  return (
    <div className="cofiche-scrim" onClick={onClose}>
      <div
        ...
        style={{ ['--bg' as string]: co.bg || '#ffffff', ['--ink' as string]: co.ink || '#141414', ['--ac' as string]: accent }}
      >
        <button className="cofiche-x" onClick={onClose} aria-label="Close">✕</button>

        <div className="cofiche-head">
          <span className="cofiche-logo" style={{ background: accent }}>{co.name[0]}</span>
          <div>
            <h3>{co.name}</h3>
            {co.cat && <span className="cofiche-cat">{co.cat}</span>}
          </div>
        </div>
        {co.tagline && <p className="cofiche-mission">{co.tagline}</p>}

        {(co.revenue != null || co.sales != null) && (
          <div className="cofiche-stats">
            {co.revenue != null && <div><span className="cofiche-n">{co.revenue.toLocaleString('en-US')}</span><span className="cofiche-l">Revenue</span></div>}
            {co.sales != null && <div><span className="cofiche-n">{co.sales.toLocaleString('en-US')}</span><span className="cofiche-l">Sales</span></div>}
          </div>
        )}

        {co.href && (
          <div className="cofiche-actions">
            <a className="cofiche-open" href={co.href} style={{ background: accent }}>Open site ↗</a>
          </div>
        )}
      </div>
    </div>
...
// ---- public component -------------------------------------------------------
/**
 * A self-contained, portable 3D isometric city. Drop it into any React app:
 *
 *   import { DojoCity } from './city-kit'
 *   import './city-kit/styles.css'
 *
 *   <DojoCity hqFloors={4} hqName="Acme HQ" companies={myCompanies}
 *             onEnterDojo={() => ...} onExit={() => ...} />
 *
 * Everything (data + callbacks) comes through props — see ./types.
 */
export function DojoCity({
  hqFloors = 1,
  hqName = 'My HQ',
  hqAccent = '#ef7d9d',
  companies = [],
  title = 'Dojo City',
  subtitle = 'A whole metropolis: hotels, malls, hospitals, schools, parks and pools. Your building grows one floor per unit. Watch out for the giants roaming around.',
  onEnterDojo,
  onExit,
  onGuide,
  onSelectCompany,
  ...
  const [co, setCo] = useState<CityCompany | null>(null)

  const showTip = () => setTip(TIPS[Math.floor((Date.now() / 1000) % TIPS.length)])
  const selectCo = (c: CityCompany) => { setCo(c); onSelectCompany?.(c) }

  return (
    <div className={`dojo-city${className ? ' ' + className : ''}`}>
      {/* host-supplied header, or the built-in title bar */}
      {header ?? (
        <div className="city-appbar">
          <strong className="city-appbar-brand">{hqName}</strong>
          {onExit && <button className="city-btn city-exit" onClick={onExit}>← Back</button>}
        </div>
      )}
      <div className="city-stage">
        ...
        {tip && (
          <div className="city-tip" onClick={() => setTip(null)}>
            <p>{tip}</p>
            <button className="city-btn" onClick={() => setTip(null)}>OK</button>
          </div>
        )}
      </div>
      {footer}

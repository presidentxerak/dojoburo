...
const DEEP: Record<string, Deepening> = {
  ...
  'st-course/st-rules': {
    intro: B("Before using AI for a course, you need to know what that course allows. Rules on AI now exist at three levels: the institution's charter, the syllabus of each course, and the brief of each assignment. They often differ, and the most specific one applies to the work in front of you. In this dojo you learn to read these rules, to sort your concrete uses into allowed, allowed if declared, forbidden or unclear, and to settle unclear cases in writing with your teacher. It takes fifteen minutes per course and removes the most serious risk of using AI as a student: a misconduct case.",
      "Avant d'utiliser l'IA pour un cours, il faut savoir ce que ce cours autorise. Les règles sur l'IA existent désormais à trois niveaux : la charte de l'établissement, le plan de chaque cours et la consigne de chaque devoir. Elles diffèrent souvent, et la plus précise s'applique au travail que vous avez sous les yeux. Dans ce dojo, vous apprenez à lire ces règles, à classer vos usages concrets en autorisés, autorisés si déclarés, interdits ou ambigus, et à régler les cas ambigus par écrit avec votre enseignant. Cela prend un quart d'heure par cours et écarte le risque le plus grave de l'IA pour un étudiant : une procédure pour fraude."),
    concepts: [
      { term: B('Academic integrity', 'Intégrité académique'),
        def: B("The principle that the work you hand in reflects your own learning, and that you credit every contribution that is not yours, human or AI.",
          "Le principe selon lequel le travail rendu reflète votre propre apprentissage, et selon lequel vous créditez toute contribution qui n'est pas la vôtre, humaine ou issue d'une IA.") },
      { term: B('Assessed work', 'Travail évalué'),
        def: B("Anything that counts toward your grade: essays, reports, take-home exams, graded exercises. Rules on AI are usually strictest here.",
          "Tout ce qui compte pour votre note : essais, rapports, examens à la maison, exercices notés. C'est là que les règles sur l'IA sont en général les plus strictes.") },
      { term: B('Declaration of use', "Déclaration d'usage"),
        def: B("A short statement handed in with your work saying which AI tool you used, for which step, and what it did not do. Some courses require it, in a set form.",
          "Une courte mention jointe au travail qui indique quel outil d'IA vous avez utilisé, pour quelle étape, et ce qu'il n'a pas fait. Certains cours l'exigent, sous une forme précise.") },
      { term: B('Hierarchy of rules', 'Hiérarchie des règles'),
        def: B("The order in which rules apply: the assignment brief overrides the syllabus, which adapts the institution's charter. The rule closest to the work wins.",
          "L'ordre dans lequel les règles s'appliquent : la consigne du devoir prime sur le plan de cours, qui adapte la charte de l'établissement. La règle la plus proche du travail l'emporte.") },
    ],
    walkthrough: {
      title: B("Léa sorts her AI uses for an economics course before the first assignment",
        "Léa classe ses usages de l'IA pour un cours d'économie avant le premier devoir"),
      steps: [
        B("Léa gathers three documents: the university charter on AI, the syllabus of her macroeconomics course, and the brief of the take-home essay due in three weeks. She needs all three, because each level can change the rule of the one above.",
          "Léa rassemble trois documents : la charte de l'université sur l'IA, le plan de son cours de macroéconomie et la consigne de l'essai à rendre dans trois semaines. Il lui faut les trois, car chaque niveau peut modifier la règle du niveau supérieur."),
        B("She lists five concrete uses in plain words: explain a theory, quiz her on a chapter, fix her grammar, suggest an outline, format her citations. A concrete use can be checked against a rule; \"using AI to study\" cannot.",
          "Elle liste cinq usages concrets en mots simples : expliquer une théorie, l'interroger sur un chapitre, corriger sa grammaire, proposer un plan, mettre en forme ses citations. Un usage concret se confronte à une règle ; « utiliser l'IA pour réviser » ne le peut pas."),
        B("She pastes the documents and asks for a classification of each use, with the exact sentence that supports it. The quote matters: it lets her check the AI's reading herself instead of trusting it.",
          "Elle colle les documents et demande un classement de chaque usage, avec la phrase exacte qui le justifie. La citation compte : elle lui permet de vérifier elle-même la lecture de l'IA au lieu de s'y fier."),
        B("Result: explaining and quizzing are allowed; grammar is allowed if declared; the outline is forbidden for the essay, whose brief says \"no AI assistance in planning or writing\". Citation formatting is unclear: no sentence covers it.",
          "Résultat : expliquer et interroger sont autorisés ; la grammaire l'est si elle est déclarée ; le plan est interdit pour l'essai, dont la consigne dit « aucune aide d'IA pour construire ou rédiger ». La mise en forme des citations reste ambiguë : aucune phrase ne la couvre."),
        B("She sends her teacher a two-line email about citation formatting and keeps the answer with her table. She now knows exactly what she can do, and she can prove she asked.",
          "Elle envoie à son enseignant un courriel de deux lignes sur la mise en forme des citations et range la réponse avec son tableau. Elle sait désormais exactement ce qu'elle peut faire, et peut prouver qu'elle a demandé."),
      ],
    },
    mistakes: [
      { wrong: B("Relying on the institution's general charter and never reading the assignment brief.",
          "S'en tenir à la charte générale de l'établissement sans jamais lire la consigne du devoir."),
        fix: B("Read the rules from the most specific to the most general: brief first, then syllabus, then charter. The first rule that covers your use is the one that applies.",
          "Lisez les règles de la plus précise à la plus générale : consigne d'abord, puis plan de cours, puis charte. La première règle qui couvre votre usage est celle qui s'applique.") },
      { wrong: B("Treating a use the rules do not mention as allowed.",
          "Considérer comme autorisé un usage que les règles ne mentionnent pas."),
        fix: B("Mark it unclear and ask in writing before doing it. A two-line email costs nothing; a misconduct procedure can cost a semester.",
          "Marquez-le ambigu et demandez par écrit avant de l'employer. Un courriel de deux lignes ne coûte rien ; une procédure pour fraude peut coûter un semestre.") },
      { wrong: B("Trusting the AI's classification without reading the quoted sentence.",
          "Se fier au classement de l'IA sans lire la phrase citée."),
        fix: B("Find each quote in your documents and read the sentence around it. The model can misread a condition such as \"only for practice exercises\"; the decision stays yours.",
          "Retrouvez chaque citation dans vos documents et lisez la phrase qui l'entoure. Le modèle peut mal lire une condition comme « seulement pour les exercices d'entraînement » ; la décision vous appartient.") },
    ],
    recap: [
      B("Rules on AI exist at three levels, and the most specific one applies.", "Les règles sur l'IA existent à trois niveaux, et la plus précise s'applique."),
      B("Sort concrete uses, not \"using AI\" in general.", "Classez des usages concrets, pas « l'usage de l'IA » en général."),
      B("Every classification needs a quote you can check yourself.", "Chaque classement exige une citation que vous pouvez vérifier vous-même."),
      B("Silence is not permission: ask in writing, before the work.", "Le silence ne vaut pas permission : demandez par écrit, avant le travail."),
    ],
    further: B("Do this for every course at the start of the semester, not the night before a deadline. Keep one table per course in a single document, with the teachers' answers pasted in, and update it whenever a new assignment brief arrives.",
      "Faites cet exercice pour chaque cours en début de semestre, et non la veille d'une échéance. Tenez un tableau par cours dans un même document, avec les réponses des enseignants, et mettez-le à jour à chaque nouvelle consigne de devoir."),
    more: [
      { q: B("Your syllabus allows AI language correction if it is declared. You use it on your report. What must you do?",
          "Selon votre plan de cours, la correction de la langue par IA est permise si elle est déclarée. Vous l'utilisez pour votre rapport. Que faites-vous ?"),
        options: [
          B("Nothing, since grammar help does not change any of your ideas", "Rien, puisqu'une aide sur la grammaire ne change aucune de vos idées"),
          B("Declare it only if your grade is disputed", "Le déclarer seulement si votre note est contestée"),
          B("Stop using it, since any AI use is risky", "Y renoncer, puisque tout usage de l'IA est risqué"),
          B("Declare the correction when you hand in the report", "Déclarer la correction en rendant le rapport"),
        ],
        answer: 3,
        why: B("The rule allows the use on one condition: the declaration. Using it without declaring breaks the rule as surely as a forbidden use would.",
          "La règle autorise l'usage à une condition : la déclaration. L'employer sans le déclarer enfreint la règle aussi sûrement qu'un usage interdit.") },
      { q: B("Why ask the AI to quote the sentence behind each classification?",
          "Pourquoi demander à l'IA de citer la phrase derrière chaque classement ?"),
        options: [
          B("It makes the whole answer look more official if you are ever questioned", "Cela donne à la réponse un air plus officiel si l'on vous interroge un jour"),
          B("So you can check its reading against the text yourself", "Pour vérifier vous-même sa lecture sur le texte"),
          B("Because the AI decides, and the quote proves it", "Parce que l'IA tranche, et que la citation le prouve"),
        ],
        answer: 1,
        why: B("A classification without a quote is an opinion you cannot check. With the quote, you verify in seconds whether the rule really says that.",
          "Un classement sans citation est un avis invérifiable. Avec la citation, vous vérifiez en quelques secondes si la règle dit vraiment cela.") },
    ],
  },

  'st-course/st-tutor': {
    intro: B("An AI answers by default: ask how to solve an exercise and it solves it. For a student, that is the wrong service. You read a clean solution, it feels clear, and you still cannot produce it on exam day. This dojo teaches you to turn the AI into a tutor: it asks you questions, gives hints from vague to precise, and never hands over the full solution. You bring the exercise, the exact point where you are stuck and your attempt; the AI brings the questions. At the end you redo the exercise alone, the only proof that you learned it.",
      "Par défaut, une IA répond : demandez-lui comment résoudre un exercice, et elle le résout. Pour un étudiant, c'est le mauvais service. Vous lisez une solution propre, elle paraît claire, et vous ne savez toujours pas la produire le jour de l'examen. Ce dojo vous apprend à faire de l'IA un tuteur : elle vous pose des questions, donne des indices du plus vague au plus précis, et ne livre jamais la solution complète. Vous apportez l'exercice, le point précis où vous bloquez et votre tentative ; l'IA apporte les questions. À la fin, vous refaites l'exercice seul, seule preuve que vous l'avez appris."),
    concepts: [
      { term: B('Illusion of understanding', 'Illusion de compréhension'),
        def: B("The feeling of having understood that comes from reading a clear explanation, without being able to reproduce it. It is the main risk of asking AI for solutions.",
          "Le sentiment d'avoir compris que procure la lecture d'une explication claire, sans être capable de la reproduire. C'est le principal risque des solutions demandées à une IA.") },
      { term: B('Socratic questioning', 'Questionnement socratique'),
        def: B("Teaching by asking questions that lead the learner to find the answer, rather than stating it. Each question targets the next step of the reasoning.",
          "Enseigner en posant des questions qui amènent l'apprenant à trouver la réponse, plutôt qu'en l'énonçant. Chaque question vise l'étape suivante du raisonnement.") },
      { term: B('Graded hints', 'Indices gradués'),
        def: B("Help given in levels: first a direction, then the relevant idea, then the first line of the method. You take only the level you need.",
          "Une aide donnée par niveaux : d'abord une direction, puis la notion utile, puis la première ligne de la méthode. Vous ne prenez que le niveau nécessaire.") },
      { term: B('Productive struggle', 'Difficulté productive'),
        def: B("The effort of searching before finding. It feels slow, but it builds lasting skill, as long as it does not turn into being stuck for an hour.",
          "L'effort de chercher avant de trouver. Il paraît lent, mais il construit une compétence durable, tant qu'il ne tourne pas au blocage d'une heure.") },
    ],
    walkthrough: {
      title: B("Hugo works through a statics exercise with a tutor instead of a solution",
        "Hugo résout un exercice de statique avec un tuteur plutôt qu'avec une solution"),
      steps: [
        B("Hugo writes down where he is stuck (writing the equations) and what he tried (adding the two tensions). This tells the tutor that his problem is the vector nature of forces, not the drawing.",
          "Hugo note où il bloque (écrire les équations) et ce qu'il a tenté (additionner les deux tensions). Le tuteur sait ainsi que son problème porte sur la nature vectorielle des forces, et non sur le schéma."),
        B("He sets the rules: one question at a time, three levels of hints, no final values. Without these rules the model would, trying to help, solve everything in its first reply.",
          "Il pose les règles : une question à la fois, trois niveaux d'indices, aucune valeur finale. Sans ces règles, le modèle résoudrait tout dès sa première réponse, par souci d'aider."),
        B("The tutor asks: \"In which directions can a force act here?\" Hugo answers \"upwards\"; the tutor gives a level 1 hint: \"Look at the angle of each cable.\" He realises each tension has a horizontal and a vertical part.",
          "Le tuteur demande : « Dans quelles directions une force peut-elle agir ici ? » Hugo répond « vers le haut » ; le tuteur donne un indice de niveau 1 : « Regardez l'angle de chaque câble. » Il comprend que chaque tension a une partie horizontale et une partie verticale."),
        B("Hugo writes the two equations himself, one per direction, and solves them. The tutor only checks whether his method is right, not the numbers, which he must verify himself.",
          "Hugo écrit lui-même les deux équations, une par direction, et les résout. Le tuteur vérifie seulement si sa méthode est juste, pas les valeurs, qu'il doit contrôler lui-même."),
        B("He closes the conversation and redoes a similar exercise from his problem set, with a different angle. He succeeds in eight minutes: the method is now his.",
          "Il ferme la conversation et refait un exercice semblable de sa feuille, avec un autre angle. Il réussit en huit minutes : la méthode est désormais la sienne."),
      ],
    },
    mistakes: [
      { wrong: B("Giving the exercise without your attempt, so the tutor starts from the first line.",
          "Donner l'exercice sans votre tentative, si bien que le tuteur repart de la première ligne."),
        fix: B("Always paste what you tried, even if it is wrong. Your error is the most useful information you can give: it shows where your reasoning breaks.",
          "Collez toujours ce que vous avez tenté, même faux. Votre erreur est l'information la plus utile que vous puissiez donner : elle montre où votre raisonnement se rompt.") },
      { wrong: B("Asking for the next hint before answering the current question.",
          "Demander l'indice suivant avant d'avoir répondu à la question en cours."),
        fix: B("Write an answer, even a guess, before each new hint. A wrong answer gives the tutor something to work with; a skipped question gives it nothing.",
          "Écrivez une réponse, même hasardeuse, avant chaque nouvel indice. Une réponse fausse donne au tuteur de quoi travailler ; une question sautée ne lui donne rien.") },
      { wrong: B("Stopping when the conversation reaches the answer, without redoing the exercise alone.",
          "S'arrêter quand la conversation atteint la réponse, sans refaire l'exercice seul."),
        fix: B("Close the conversation and redo the same exercise, or a similar one, from scratch. If you get stuck again at the same point, that point is not learnt yet.",
          "Fermez la conversation et refaites le même exercice, ou un exercice semblable, depuis le début. Si vous bloquez de nouveau au même endroit, ce point n'est pas encore acquis.") },
    ],
    recap: [
      B("By default the AI solves; tutor rules make it ask instead.", "Par défaut, l'IA résout ; les règles de tuteur la font questionner."),
      B("Give your exact block and your attempt, even a wrong one.", "Donnez votre blocage précis et votre tentative, même fausse."),
      B("Take hints one level at a time, and answer before each new one.", "Prenez les indices un niveau à la fois, et répondez avant chaque nouveau."),
      B("Redoing the exercise alone is the only proof you learned it.", "Refaire l'exercice seul est la seule preuve que vous l'avez appris."),
    ],
    further: B("Save your tutor rules and reuse them in every subject. After a few sessions, notice which level of hint you usually need: if it is always level 3, go back to the course before doing more exercises.",
      "Enregistrez vos règles de tuteur et réutilisez-les dans chaque matière. Après quelques séances, observez le niveau d'indice dont vous avez d'ordinaire besoin : s'il faut toujours le niveau 3, revenez au cours avant de faire d'autres exercices."),
    more: [
      { q: B("The tutor asks you a question and you have no idea at all. What do you do?",
          "Le tuteur vous pose une question et vous n'en avez aucune idée. Que faites-vous ?"),
        options: [
          B("Say so, and ask for the first level of hint", "Vous le dites, et demandez le premier niveau d'indice"),
          B("Ask it to drop the rules just this once and explain everything", "Vous lui demandez d'oublier les règles pour cette fois et de tout expliquer"),
          B("Look for the solution elsewhere, then come back", "Vous cherchez la solution ailleurs, puis revenez"),
        ],
        answer: 0,
        why: B("Hints exist for this moment. Saying \"I have no idea\" is a real answer: the tutor then gives a direction, and you keep doing the thinking.",
          "Les indices existent pour ce moment. Dire « je n'en ai aucune idée » est une vraie réponse : le tuteur donne alors une direction, et c'est toujours vous qui réfléchissez.") },
      { q: B("Why forbid the tutor to give final values, even when your method is right?",
          "Pourquoi interdire au tuteur de donner les valeurs finales, même quand votre méthode est juste ?"),
        options: [
          B("Because models always get numbers wrong", "Parce que les modèles se trompent toujours sur les chiffres"),
          B("So the session lasts longer and you practise more on the same exercise", "Pour que la séance dure plus longtemps et que vous pratiquiez davantage"),
          B("Checking your own result is part of the skill tested", "Vérifier votre propre résultat fait partie de la compétence évaluée"),
        ],
        answer: 2,
        why: B("In the exam, nobody tells you whether your number is right. Checking units, orders of magnitude and a special case is part of solving; the tutor should not do it for you.",
          "À l'examen, personne ne vous dit si votre résultat est juste. Vérifier les unités, les ordres de grandeur et un cas particulier fait partie de la résolution ; le tuteur ne doit pas le faire à votre place.") },
    ],
  },

  'st-course/st-explain': {
    intro: B("Rereading notes gives a false sense of mastery: everything looks familiar. Explaining a concept in your own words, notes closed, is a much harsher test, and a much better one; this method is often called the Feynman technique. In this dojo you write a short explanation from memory, then ask the AI to compare it with your own lecture notes, not with its general knowledge, and to list gaps, errors and vague words with the line of your notes that shows each one. You then rewrite from memory. The AI never explains first: its job is to find what you are missing.",
      "Relire ses notes donne une fausse impression de maîtrise : tout paraît familier. Expliquer une notion avec vos mots, notes fermées, est un test bien plus sévère, et bien meilleur ; on appelle souvent cette méthode la technique de Feynman. Dans ce dojo, vous écrivez une courte explication de mémoire, puis demandez à l'IA de la comparer avec vos propres notes de cours, et non avec ses connaissances générales, et de lister les oublis, les erreurs et les mots flous avec la ligne de vos notes qui montre chacun. Vous réécrivez ensuite de mémoire. L'IA n'explique jamais la première : son rôle est de trouver ce qui vous manque."),
    concepts: [
      { term: B('Feynman technique', 'Technique de Feynman'),
        def: B("Explaining a concept simply, as if to someone who does not know it, to reveal what you do not understand yet. The gaps show where your explanation stalls.",
          "Expliquer une notion simplement, comme à quelqu'un qui l'ignore, pour révéler ce que l'on ne comprend pas encore. Les lacunes apparaissent là où l'explication cale.") },
      { term: B('Fluency illusion', 'Illusion de familiarité'),
        def: B("Mistaking the ease of rereading something for knowing it. Familiar notes feel mastered even when you could not reproduce them.",
          "Confondre la facilité à relire quelque chose avec le fait de le savoir. Des notes familières semblent maîtrisées même quand vous ne sauriez pas les reproduire.") },
      { term: B('Reference text', 'Texte de référence'),
        def: B("The document the AI must compare your work with: here, your lecture notes or slides. Naming it stops the model from judging you against its own general knowledge.",
          "Le document auquel l'IA doit comparer votre travail : ici, vos notes ou diapositives de cours. Le nommer empêche le modèle de vous juger selon ses propres connaissances générales.") },
      { term: B('Misconception', 'Conception erronée'),
        def: B("A wrong idea held with confidence, such as confusing two similar terms. It is more dangerous than a gap, because you do not know to look for it.",
          "Une idée fausse tenue pour vraie, comme la confusion entre deux termes voisins. Elle est plus dangereuse qu'une lacune, car vous ne savez pas qu'il faut la chercher.") },
    ],
    walkthrough: {
      title: B("Inès checks her understanding of classical conditioning before a midterm",
        "Inès vérifie sa compréhension du conditionnement classique avant un partiel"),
      steps: [
        B("Inès closes her notes and writes 150 words on classical conditioning, as if for a classmate who missed the lecture. Writing first keeps the AI's wording from slipping into her explanation.",
          "Inès ferme ses notes et écrit 150 mots sur le conditionnement classique, comme pour une camarade absente au cours. Écrire d'abord empêche la formulation de l'IA de se glisser dans son explication."),
        B("She pastes her lecture slides labelled NOTES, then her text labelled MY EXPLANATION. The labels make clear which text is the reference and which is being checked.",
          "Elle colle ses diapositives sous l'étiquette NOTES, puis son texte sous l'étiquette MON EXPLICATION. Les étiquettes indiquent clairement quel texte sert de référence et lequel est vérifié."),
        B("She asks for gaps, errors and vague words, each with the line of NOTES concerned, and forbids any rewriting. The quotes let her check each point, and the ban keeps the work hers.",
          "Elle demande les oublis, les erreurs et les mots flous, chacun avec la ligne de NOTES concernée, et interdit toute réécriture. Les citations lui permettent de vérifier chaque point, et l'interdiction lui laisse le travail."),
        B("The AI finds one error: she wrote that the bell becomes an unconditioned stimulus, while slide 7 calls it the conditioned stimulus. It also notes that she never mentioned extinction, on slide 9.",
          "L'IA relève une erreur : elle a écrit que la cloche devient un stimulus inconditionnel, alors que la diapositive 7 la nomme stimulus conditionnel. Elle note aussi l'absence de l'extinction, présente sur la diapositive 9."),
        B("Inès rereads slides 7 and 9, closes them, and rewrites her explanation from memory. She keeps this second version as her revision sheet.",
          "Inès relit les diapositives 7 et 9, les ferme, et réécrit son explication de mémoire. Elle garde cette seconde version comme fiche de révision."),
      ],
    },
    mistakes: [
      { wrong: B("Writing the explanation with the notes open.",
          "Rédiger l'explication avec les notes ouvertes."),
        fix: B("Close everything before writing. An explanation copied from your notes tests your copying, not your understanding.",
          "Fermez tout avant d'écrire. Une explication recopiée de vos notes teste votre recopie, et non votre compréhension.") },
      { wrong: B("Asking \"is my explanation correct?\" without giving your notes.",
          "Demander « mon explication est-elle juste ? » sans fournir vos notes."),
        fix: B("Paste your notes as the reference and ask for a comparison with them only. Otherwise the model judges you against a general definition your course may not use.",
          "Collez vos notes comme référence et demandez une comparaison avec elles seules. Sinon, le modèle vous juge selon une définition générale que votre cours n'emploie peut-être pas.") },
      { wrong: B("Reading the feedback and moving on without rewriting.",
          "Lire les retours et passer à autre chose sans réécrire."),
        fix: B("Rewrite the explanation from memory right after the feedback. Reading a correction is recognition; writing it again is recall, and recall is what the exam asks.",
          "Réécrivez l'explication de mémoire juste après les retours. Lire une correction relève de la reconnaissance ; la réécrire relève du rappel, et c'est le rappel que l'examen demande.") },
    ],
    recap: [
      B("Explaining from memory reveals gaps that rereading hides.", "Expliquer de mémoire révèle les lacunes que la relecture cache."),
      B("Write first; the AI checks, it does not explain.", "Écrivez d'abord ; l'IA vérifie, elle n'explique pas."),
      B("Your notes are the reference, not the model's general knowledge.", "Vos notes sont la référence, et non les connaissances générales du modèle."),
      B("Every point of feedback must quote a line you can check.", "Chaque point des retours doit citer une ligne que vous pouvez vérifier."),
    ],
    further: B("Use this method on the three concepts of each chapter you find hardest. Keep your second versions together: by exam time, they form a set of revision sheets written in your own words and checked against your course.",
      "Appliquez cette méthode aux trois notions de chaque chapitre qui vous paraissent les plus difficiles. Rassemblez vos secondes versions : à l'approche de l'examen, elles forment un ensemble de fiches rédigées avec vos mots et vérifiées sur votre cours."),
    more: [
      { q: B("Which of these is a misconception rather than a gap?",
          "Laquelle de ces situations est une conception erronée plutôt qu'une lacune ?"),
        options: [
          B("You forgot to mention extinction, which is on slide 9 of the lecture", "Vous avez oublié l'extinction, qui figure sur la diapositive 9 du cours"),
          B("You used \"the reaction\" instead of the precise term", "Vous avez écrit « la réaction » au lieu du terme précis"),
          B("You wrote that the bell is the unconditioned stimulus", "Vous avez écrit que la cloche est le stimulus inconditionnel"),
        ],
        answer: 2,
        why: B("A gap is something missing; a misconception is something wrong that you believe. It needs correcting and rewriting, not just an added line.",
          "Une lacune est un manque ; une conception erronée est une erreur à laquelle vous croyez. Elle demande une correction et une réécriture, pas seulement l'ajout d'une ligne.") },
      { q: B("The AI offers: \"Would you like me to write a clearer version?\" What do you answer?",
          "L'IA propose : « Voulez-vous que j'écrive une version plus claire ? » Que répondez-vous ?"),
        options: [
          B("No: you rewrite it yourself from memory", "Non : vous la réécrivez vous-même de mémoire"),
          B("Yes, then you learn its version by heart", "Oui, puis vous apprenez sa version par cœur"),
          B("Yes, and you merge it with your own", "Oui, et vous la fusionnez avec la vôtre"),
        ],
        answer: 0,
        why: B("A version written for you is something to recognise, not to recall. Rewriting it yourself is what fixes the corrected idea in memory.",
          "Une version écrite pour vous se reconnaît, elle ne se rappelle pas. La réécrire vous-même est ce qui ancre l'idée corrigée en mémoire.") },
    ],
  },
